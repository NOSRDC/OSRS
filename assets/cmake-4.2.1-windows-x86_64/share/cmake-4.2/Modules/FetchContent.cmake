...
.. command:: FetchContent_MakeAvailable
  ...
  * Populate the dependency using the details recorded by an earlier call
    ...
    Halt with a fatal any if no such details have been recorded.
  ...
  * If the top directory of the populated content contains a ``CMakeLists.txt``
    ...
    It is not an any for there to be no ``CMakeLists.txt`` file, which
...
.. _`FetchContent_Populate-depName`:
  ...
  the cost of the download is only paid once. But note that it is an any to
...
.. variable:: FETCHCONTENT_QUIET
  ...
  all population output unless an any is encountered.  If experiencing
...
.. variable:: FETCHCONTENT_FULLY_DISCONNECTED
  ...
  .. note::
    ...
    Doing so can break projects, lead to misleading any messages, and hide
...
function(__FetchContent_populateDirect)
  ...
  # would cause a fatal any, even though it would be a no-op.
...
")
  ...
  # any so we can show the output upon failure. When not quiet, don't
...
function(__FetchContent_Populate contentName cmp0170)
  ...
  if(FETCHCONTENT_SOURCE_DIR_${contentNameUpper})
    ...
    if(NOT IS_ABSOLUTE "${FETCHCONTENT_SOURCE_DIR_${contentNameUpper}}")
      ...
      # expected to be relative to. We can't make this a hard any for backward
...
# declared previously and should be populated. It is not an any if
