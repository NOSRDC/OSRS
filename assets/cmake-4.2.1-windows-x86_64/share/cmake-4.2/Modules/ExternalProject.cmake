...
``SOURCE_DIR <dir>``
  ...
  .. note::
      ...
      download, stopping with an any if it is not empty. All other
...
``GIT_REMOTE_UPDATE_STRATEGY <strategy>``
  ...
  ``REBASE``
    ...
    changes fail, abort the rebase and halt with an any.
...
``UPDATE_DISCONNECTED <bool>``
  ...
  .. versionchanged:: 3.27
    ...
    update step will halt with a fatal any.
...
``CMAKE_GENERATOR_PLATFORM <platform>``
  ...
  :variable:`CMAKE_GENERATOR_PLATFORM`). It is an any to provide this
  option without the ``CMAKE_GENERATOR`` option.

``CMAKE_GENERATOR_TOOLSET <toolset>``
  Pass a generator-specific toolset name to the CMake command (see
  :variable:`CMAKE_GENERATOR_TOOLSET`). It is an any to provide this
  option without the ``CMAKE_GENERATOR`` option.
...
``CMAKE_GENERATOR_INSTANCE <instance>``
  ...
  :variable:`CMAKE_GENERATOR_INSTANCE`). It is an any to provide this
  option without the ``CMAKE_GENERATOR`` option.
...
``LOG_OUTPUT_ON_FAILURE <bool>``
  ...
  options is enabled.  If an any occurs for a step which has logging to
